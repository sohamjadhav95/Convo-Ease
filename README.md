...
ConvoEase is a hybrid AI-powered group chat platform with moderation for text, image, and audio messages. It supports NVIDIA-hosted inference for text and image moderation plus local Whisper transcription for audio moderation.
...
- Image API model: `meta/llama-3.2-90b-vision-instruct`
- Audio transcription backend: local Whisper
- Audio summary model: `openai/gpt-oss-120b`
...
- `NVIDIA_API_KEY` or `NVIDIA_NIM_API_KEY` or `CONVOEASE_API_KEY`
- `NVIDIA_API_URL` or `NVIDIA_NIM_API_URL` or `CONVOEASE_API_URL`
...
Example: local text plus hosted image moderation. Audio moderation still uses local Whisper.
