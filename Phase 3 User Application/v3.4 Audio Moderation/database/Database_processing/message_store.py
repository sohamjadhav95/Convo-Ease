...
class MessageStore:
    ...
    @staticmethod
    def save_message(group_id, username, message, status, reason="", summary="", media_url=""):
        """Save a new message to the database.

        Args:
            group_id:  Group the message belongs to.
            username:  Sender's username.
            message:   The stored message content (for images: "[IMAGE]").
            status:    "PASS" or "FLAGGED".
            reason:    Moderation block reason (for FLAGGED messages).
            summary:   AI summary of the image (stored separate from the display message).
            media_url: Server-relative URL to the saved media file (e.g. "/media/image/abc.jpg").
                       Empty string for text messages.
        """
        new_msg = {
            ...
            "status":     status,
            "reason":     reason,
            "summary":    summary,      # AI image summary (empty for text messages)
